class MiqRequest < ActiveRecord::Base
  ...
  def initialize_attributes
    ...
    miq_approvals.first.update_attribute(:stamper_name, "default")
  ...
  # def approved?
  #   self.miq_approvals.all? { |a| a.state == "approved" }
  # end

  def approved?
    miq_approvals.order(:id).each do |a|
      _log.info("Request: stamper_name '#{a.stamper_name}' state '#{a.state}'")
      return false if a.state != "approved"
    end
  end
  ...
  def add_approver(approver)
    _log.info("MIQ(MiqRequest.add_approver) miq_approvals count: [#{miq_approvals.count}]\n")
    miq_approvals << build_default_approval
    miq_approvals.last.update_attribute(:stamper_name, approver)
    _log.info("MIQ(MiqRequest.add_approver) miq_approvals count: [#{miq_approvals.count}]\n")
  end
  ...
  #  miq_approvals.first || build_default_approval
    miq_approvals.order(:id).first || build_default_approval
  end
